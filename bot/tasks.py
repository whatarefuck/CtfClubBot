                    ...
                    try:
                        ...
                        for task in user.tasks:
                            ...
                            if not task.completed:
                                ...
                                user.lives -= 1
                                logger.info(f"Decreased HP for user: {user.username}. Current HP: {user.lives}")
                                
                                if user.lives <= 0:
                                    # Если HP достигло 0, уведомляем админов
                                    await Notifications._say_admins(f"User {user.username} has reached 0 HP!")
                                    logger.info(f"Notified admins about {user.username} reaching 0 HP.")
